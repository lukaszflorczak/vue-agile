	...
	watch: {
		...
		dragDistance () {
			if (this.mouseDown) {
				...
				if (dragDistance > this.dragSwipeDistance && this.canGoToPrev) {
					this.goToPrev()
                    this.dragSwipeDistance = this.dragSwipeDistance + this.settings.swipeDistance
				}

				if (dragDistance < -1 * this.dragSwipeDistance && this.canGoToNext) {
					this.goToNext()
                    this.dragSwipeDistance = this.dragSwipeDistance + this.settings.swipeDistance
				}
			}
